...
| `urlSchemes` | `string[]?` | Custom URL schemes on macOS, Windows, and Linux. |
| `fileAssociations` | `FileAssociation[]?` | Document associations on macOS, Windows, and Linux. |
| `singleInstance` | `boolean?` | Forward secondary launches on Windows and Linux; defaults on when URL schemes are configured. |

### URL Schemes

Custom URL schemes are registered through Launch Services on macOS, the
current-user registry on Windows, and the installed desktop entry on Linux.
On macOS, install the built app in `/Applications` or `~/Applications` so
Launch Services can register it.
...
File associations are supported on macOS, Windows, and Linux. Extensions do
not include the leading dot. The optional `.icns` icon applies to macOS.
...
On Windows and Linux, secondary launches forward deep links and file opens to
the running application when `singleInstance` is enabled. Packaged builds are
required to test operating-system registration.
...
- **Dev builds ignore `bytecode`** (`hutch electrobun dev` / `--env=dev`) and
  always ship source, so sourcemaps and the debugger keep working. In watch
  mode, Hutch rebuilds and relaunches the app from that source. Bytecode applies
  to canary and stable builds.
...
Watch mode rebuilds and relaunches the whole app; it is distinct from
webview-only hot module replacement. See [Hot Reloading](/electrobun/guides/hot-reloading/)
for the main-process, Vite, and Warren reload boundaries.
